...
function getImageIdsFromDateRange(dateFrom, dateTo, timeFrom, timeTo) {
  const from = time.getDateTimeToQueryTime(dateFrom, timeFrom);
  const to = time.getDateTimeToQueryTime(dateTo, timeTo);

  const params = {
    queryString: `lastPublishDateTime:>${from} AND lastPublishDateTime:<${to}`,
    maxResults: 100,
    aspects: [ 'images' ]
  };

  return searchDeeper(params)
    .then(r => extractArticles(r))
    .then(r => extractImages(r))
    .catch(err => {
        throw err;
    });
}
...
function extractImages(results){
  return results
          .filter(r => (r.images[0] && r.images[0].url))
          .map(r => (r.images[0] && r.images[0].url) ? getImageIdFromUrl(r.images[0].url) : undefined );
}

function getImageIdFromUrl(url){
  const idMatch = /%2F([a-z0-9]{8}.*)\?source/gm.exec(url);
  return (idMatch[1]) ? idMatch[1] : undefined;
}
